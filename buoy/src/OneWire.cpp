...
void OneWire::begin(I2C_ID_t i2c)
{
...
void OneWire::begin(I2C_ID_t i2c, uint8_t address)
{
	// Address is determined by two pins on the DS2482 AD1/AD0
	// Pass 0b00, 0b01, 0b10 or 0b11
...
{
	I2C_Error_t error = I2C_Transmit(_i2c, mAddress, &data, 1, I2C_DEFAULT_TIME_OUT);
	if (error != I2C_ERROR_NONE) {
		Trace(1,"writeByte transmit error: 0X%02x", error);
	}
}
...
{
	uint8_t res = 0;
	I2C_Error_t error = I2C_Receive(_i2c, mAddress, &res, 1, I2C_DEFAULT_TIME_OUT);
	if (error != I2C_ERROR_NONE) {
		Trace(1,"readByteError: 0X%02x", error);
	}
//	Trace(1,"readByte: 0X%02x", res);
	return res;
}
...
{
	uint8_t data[2];
	data[0] = DS2482_COMMAND_SRP;
	data[1] = readPointer;
	I2C_Error_t error = I2C_Transmit(_i2c, mAddress, &data[0], 2, I2C_DEFAULT_TIME_OUT);
	if (error != I2C_ERROR_NONE) {
		Trace(1,"setReadPointer transmit error: 0X%02x", error);
	}
}
...
uint8_t OneWire::waitOnBusy()
{
	...
	if (status & DS2482_STATUS_BUSY) {
		...
		Trace(1,"mError: DS2482_ERROR_TIMEOUT");
	}
...
void OneWire::writeConfig(uint8_t config)
{
	...
	uint8_t data[2];
	data[0] = DS2482_COMMAND_WRITECONFIG;
	data[1] = config | (~config)<<4;
	I2C_Error_t error = I2C_Transmit(_i2c, mAddress, &data[0], 2, I2C_DEFAULT_TIME_OUT);
	if (error != I2C_ERROR_NONE) {
		Trace(1,"writeConfig transmit error: 0X%02x", error);
	}

	// This should return the config bits without the complement
	if (readConfig() != config){
		mError = DS2482_ERROR_CONFIG;
		Trace(1,"mError: DS2482_ERROR_CONFIG");
	}
}
...
uint8_t OneWire::wireReset()
{
	...
	if (status & DS2482_STATUS_SD)
	{
		...
		Trace(1,"mError: DS2482_ERROR_SHORT");
	}
...
void OneWire::wireWriteByte(uint8_t byte, uint8_t power)
{
	waitOnBusy();
	if (power)
		setStrongPullup();
	...
	uint8_t data[2];
	data[0] = DS2482_COMMAND_WRITEBYTE;
	data[1] = byte;
	I2C_Error_t error = I2C_Transmit(_i2c, mAddress, &data[0], 2, I2C_DEFAULT_TIME_OUT);
	if (error != I2C_ERROR_NONE) {
		Trace(1,"wireWriteByte transmit error: 0X%02x", error);
	}
}
...
void OneWire::wireWriteBit(uint8_t bit, uint8_t power)
{
	waitOnBusy();
	if (power)
		setStrongPullup();
	...
	uint8_t data[2];
	data[0] = DS2482_COMMAND_SINGLEBIT;
	data[1] = bit ? 0x80 : 0x00;
	I2C_Error_t error = I2C_Transmit(_i2c, mAddress, &data[0], 2, I2C_DEFAULT_TIME_OUT);
	if (error != I2C_ERROR_NONE) {
		Trace(1, "wireWriteBit transmit error: 0X%02x", error);
	}
}
...
uint8_t OneWire::wireSearch(uint8_t *address)
{
	...
	for(uint8_t i=0;i<64;i++)
	{
		...
		uint8_t data[2];
		data[0] = DS2482_COMMAND_TRIPLET;
		data[1] = direction ? 0x80 : 0x00;
		I2C_Error_t error = I2C_Transmit(_i2c, mAddress, &data[0], 2, I2C_DEFAULT_TIME_OUT);
		if (error != I2C_ERROR_NONE) {
			Trace(1, "wireWriteBit transmit error: 0X%02x", error);
		}
