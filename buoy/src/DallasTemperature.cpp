...
void DallasTemperature::writeScratchPad(const uint8_t* deviceAddress,
	...
	OS_Sleep(20); // <--- added 20ms delay to allow 10ms long EEPROM write operation (as specified by datasheet)

    if (parasite) {
//		activateExternalPullup();
		OS_Sleep(10); // 10ms delay
...
bool DallasTemperature::requestTemperaturesByAddress(
	...
	uint8_t _bitResolution = getResolution(deviceAddress);
	if (_bitResolution == 0) {
...
void DallasTemperature::blockTillConversionComplete(uint8_t _bitResolution) {

	int delms = millisToWaitForConversion(_bitResolution);
	if (checkForConversion && !parasite) {
		unsigned long now = HAL_UptimeMs();
		while (!isConversionComplete() && (HAL_UptimeMs() - delms < now))
			;
	} else {
//        activateExternalPullup();
		OS_Sleep(delms);
...
uint64_t DallasTemperature::HAL_UptimeMs(void)
{
...
int16_t DallasTemperature::millisToWaitForConversion(uint8_t _bitResolution) {
